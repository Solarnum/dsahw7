/*
 * Authors: Mark Schlottke && Charlie Beckwith
 */
import java.io.File;
...
public class Sudoku
{
  ...
  private static class SudokuGame
  {
              ...
              if (entry.possibleValues.size() == 1)
                {
                  rows.get(row).set(rows.get(row).indexOf(entry),
                                    new Entry(entry.row, entry.column,
                                              entry.possibleValues.get(0)));
    ...
    public void tupleRule(Hashtable<Integer, ArrayList<Entry>> table)
    {
      int cellnum;
      ArrayList<Entry> finishedEntries = new ArrayList<Entry>();
      Hashtable<Integer, ArrayList<Entry>> tupleCandidates;

      for (cellnum = 0; cellnum < table.size(); cellnum++)
        {
          ...
          for (int i = 1; i <= 9; i++)
            {
              ...
              for (Entry entry : table.get(cellnum))
                {
                  ...
                  if (i != i2)
                    {
                      ...
                      if (tupleCandidates.get(i).contains(tupleCandidates.get(i2).get(0))
                          && tupleCandidates.get(i).contains(tupleCandidates.get(i2).get(1)))
                        {
                          ...
                          columns.get(e1.column).set(columns.get(e1.column).indexOf(e1),
                                                     e1);
                          grids.get(e1.grid).set(grids.get(e1.grid).indexOf(e1),
                                                 e1);

                          rows.get(e2.row).set(rows.get(e2.row).indexOf(e2), e2);
                          columns.get(e2.column).set(columns.get(e2.column).indexOf(e2),
                                                     e2);
                          grids.get(e2.grid).set(grids.get(e2.grid).indexOf(e2),
                                                 e2);
                        }
                    }
                }
